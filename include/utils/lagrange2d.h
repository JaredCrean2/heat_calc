...
class LagrangeEvaluator2DTPToTP
{
  public:
    using Index = LagrangeBasis::Index;

    LagrangeEvaluator2DTPToTP(const std::vector<Real>& pts_in,
...
class LagrangeEvaluator2DTPFlatToTP
{
  public:
    using Index = LagrangeBasis::Index;
...
class LagrangeEvaluator2DTPToTPFlat
{
  public:
    using Index = LagrangeBasis::Index;

    LagrangeEvaluator2DTPToTPFlat(const std::vector<Real> pts_in,
                                const std::vector<Real> pts_out,
    ...
    void interpolateVals(ArrayIn& vals_in, ArrayOut& vals_out) const
    {
      ...
      for (Index i_out=0; i_out < getNumTPPointsOut(); ++i_out)
        ...
        {
          auto node = m_nodemap_out[i_out][j_out];
          vals_out[node] = 0;
          for (Index i_in=0; i_in < getNumPointsIn(); ++i_in)
            for (Index j_in=0; j_in < getNumPointsIn(); ++j_in)
          ...
          for (Index i_in=0; i_in < getNumPointsIn(); ++i_in)
            for (Index j_in=0; j_in < getNumPointsIn(); ++j_in)
            {
              vals_out[node][0] += m_derivs[i_out][i_in]*
                m_vals[j_out][j_in]*vals_in[i_in][j_in];
              vals_out[node][1] += m_vals[i_out][i_in]*
                m_derivs[j_out][j_in]*vals_in[i_in][j_in];
            }
        }
    }
...
class LagrangeEvaluator2DTPFlatToTPFlat
{
  public:
    using Index = LagrangeBasis::Index;

    LagrangeEvaluator2DTPFlatToTPFlat(const std::vector<Real>& pts_in,
                        const std::vector<Real>& pts_out,
                        const ArrayType<LocalIndex, 2>& nodemap_in,
                        const ArrayType<LocalIndex, 2>& nodemap_out) :
      m_vals(lagrange_memoizer.getValues(pts_in, pts_out)), 
...
class LagrangeEvaluator2DTPToNonTP
{
  public:
    using Index = LagrangeBasis::Index;

    template <typename Array2D>
    LagrangeEvaluator2DTPToNonTP(const std::vector<Real>& pts_in,
                                 const Array2D& pts_out) :
      m_vals(boost::extents[pts_out.size()][pts_in.size()][2]), 
      m_derivs(boost::extents[pts_out.shape()[0]][pts_in.size()][2])
    {
...
// non-tensor product output version
class LagrangeEvaluator2DTPFlatToNonTP
{
  public:
    using Index = LagrangeBasis::Index;

    template <typename Array2D>
    LagrangeEvaluator2DTPFlatToNonTP(const std::vector<Real>& pts_in,
                               const Array2D& pts_out,
                               const ArrayType<LocalIndex, 2>& nodemap_in) :
      m_vals(boost::extents[pts_out.size()][pts_in.size()][2]), 
      m_derivs(boost::extents[pts_out.shape()[0]][pts_in.size()][2]),
