...
class HVACModelDoubleSpline : public HVACModel
{
    ...
    Real enforceTemperatureLimitDotTair(Real interior_temp, Real load_flux, Real load_flux_dot) override
    {
      ...
      } else
      {
        ...
        if (interior_temp >= params[0] && interior_temp <= params[1])
        {
          ...
          spline.setupSpline(params[0], params[1], params[2], params[3], params[4], params[5]);
          Real hvac_flux_dot;
          return spline.evalDot(interior_temp, 1, hvac_flux_dot);
        } else
        {
         return  m_switch_model.enforceTemperatureLimitDotTair(interior_temp, load_flux, load_flux_dot);
        }
  ...
  private:
    std::array<Real, 6> getSplineParams(Real interior_temp, Real load_flux)
    {
      ...
      Real flux_expansion_dot  = m_switch_model.enforceTemperatureLimitDotTair(t_expansion, load_flux, load_flux_dot);
                             ...
                             const std::array<Real, 6> params_bar)
    {
      ...
      Real load_flux_dot_bar = 0;
      m_switch_model.enforceTemperatureLimitDotTair_rev(t_expansion, load_flux, load_flux_dot, load_flux_dot_bar, flux_expansion_dot_bar);  //TODO: unnecessary?
