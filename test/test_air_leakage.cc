...
void test_finite_difference(Heat::AirLeakageModel& model, Real t_interior, Real t_exterior)
{
  Real eps = 1e-7;
  Real flux1 = model.computeAirLeakagePower(t_interior, t_exterior);
  Real flux2 = model.computeAirLeakagePower(t_interior + eps, t_exterior);
  Real deriv_fd = (flux2 - flux1)/eps;

  Real deriv_ad;
  Real flux3 = model.computeAirLeakagePowerDot(t_interior, t_exterior, deriv_ad);
...
TEST(AirLeakageNatural, Value)
{
  ...
  EXPECT_LT(model.computeAirLeakagePower(t_interior, t_exterior), 0);
...
TEST(AirLeakagePressure, Value)
{
  ...
  EXPECT_NEAR(model.computeAirLeakagePower(t_interior, t_exterior), (1 - efficiency) * (t_interior - t_exterior) * flow_rate * rho * cp, 1e-13);
  EXPECT_LT(model.computeAirLeakagePower(t_interior, t_exterior), 0);
